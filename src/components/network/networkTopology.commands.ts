  ...
  return [
    ...
    },
    {
      ...
      cmds: [
        ...
        ['ip ssh time-out <s>', isTR ? 'SSH zaman aşımı' : 'Set SSH timeout'],
        ['ip ssh authentication-retries <n>', isTR ? 'SSH deneme sayısı' : 'SSH retry limit'],
        ...
        ['vtp password <pass>', isTR ? 'VTP şifresi' : 'Set VTP password'],
        ['spanning-tree mode {m}', isTR ? 'STP modu' : 'Set STP mode'],
        ['spanning-tree vlan <id> priority <val>', isTR ? 'VLAN STP önceliği' : 'VLAN STP priority'],
        ['spanning-tree vlan <id> root', isTR ? 'VLAN STP root' : 'VLAN STP root'],
        ...
        ['ipv6 unicast-routing', isTR ? 'IPv6 yönlendirme' : 'Enable IPv6 routing'],
        ['crypto key generate rsa', isTR ? 'RSA anahtarı üret (SSH)' : 'Generate RSA keys'],
        ['snmp-server community <c>', isTR ? 'SNMP topluluğu' : 'Set SNMP community'],
        ['archive', isTR ? 'Arşiv yapılandırması' : 'Archive configuration'],
        ['alias <mode> <n> <c>', isTR ? 'Komut takma adı' : 'Command alias'],
    ...
    },
    {
      ...
      cmds: [
        ...
        ['switchport mode dot1q-tunnel', isTR ? 'Dot1q tunnel' : 'Dot1q tunnel mode'],
        ...
        ['ipv6 address <ip>/<p>', isTR ? 'IPv6 adresi ata' : 'Set IPv6 address'],
        ['no ip address', isTR ? 'IP adresini sil' : 'Remove IP address'],
        ['ip default-gateway <ip>', isTR ? 'Arayüz gateway' : 'Interface gateway'],
        ['no ip default-gateway', isTR ? 'Gateway sil' : 'Remove gateway'],
        ['ip helper-address <ip>', isTR ? 'DHCP relay' : 'Set DHCP relay'],
        ['no ip helper-address', isTR ? 'DHCP relay sil' : 'Remove DHCP relay'],
        ['ip verify source', isTR ? 'IP Source Guard' : 'Enable Source Guard'],
        ['ip dhcp snooping trust', isTR ? 'Güvenli DHCP portu' : 'DHCP snooping trust'],
        ['ip arp inspection trust', isTR ? 'Güvenli ARP portu' : 'ARP inspection trust'],
        ['storm-control {b|m|u} lvl', isTR ? 'Fırtına kontrolü' : 'Storm control'],
        ['power inline {auto|static}', isTR ? 'PoE yapılandırması' : 'Configure PoE'],
        ['bandwidth <kbps>', isTR ? 'Bant genişliği' : 'Set bandwidth'],
        ['delay <tens-of-ms>', isTR ? 'Gecikme süresi' : 'Set delay'],
        ['mls qos trust {cos|dscp}', isTR ? 'QoS güven durumu' : 'Set QoS trust'],
        ['mls qos cos <val>', isTR ? 'Default CoS değeri' : 'Set default CoS'],
        ['cdp enable', isTR ? 'CDP\'yi aç (arayüz)' : 'Enable CDP (interface)'],
        ['no udld', isTR ? 'UDLD\'yi kapat' : 'Disable UDLD'],
        ['no ip proxy-arp', isTR ? 'Proxy ARP kapat' : 'Disable proxy ARP'],
        ['no keepalive', isTR ? 'Keepalive kapat' : 'Disable keepalive'],
        ['load-interval <sec>', isTR ? 'İstatistik aralığı' : 'Set load interval'],
        ['carrier-delay <ms>', isTR ? 'Carrier gecikmesi' : 'Set carrier delay'],
    ...
    },
    {
      ...
      cmds: [
        ...
        ['transport output {p}', isTR ? 'Çıkış protokolleri' : 'Output protocols'],
        ...
        ['no history', isTR ? 'Geçmişi kapat' : 'Disable history'],
        ['exec', isTR ? 'EXEC aktif' : 'Enable EXEC'],
        ['no exec', isTR ? 'EXEC pasif' : 'Disable EXEC'],
        ['autocommand <cmd>', isTR ? 'Otomatik komut' : 'Set auto-command'],
        ['no autocommand', isTR ? 'Otomatik komut sil' : 'Remove auto-command'],
        ['transport preferred {ssh|telnet|none}', isTR ? 'Tercih edilen protokol' : 'Preferred protocol'],
        ['privilege level <0-15>', isTR ? 'Yetki seviyesi' : 'Set privilege level'],
        ['session-limit <n>', isTR ? 'Oturum sınırı' : 'Session limit'],
        ['access-class <n> {in|out}', isTR ? 'ACL uygula (line)' : 'Apply access-class'],
        ['lockable', isTR ? 'Kilitlenebilir hat' : 'Enable lockable'],
      ]
    },
    {
      id: 'router',
