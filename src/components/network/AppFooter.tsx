...
export function AppFooter({ state, selectedDevice, activeDeviceId, activeDeviceName, isDark, isCLIActive = true }: AppFooterProps) {
 ...
 className={`mt-6 rounded-[2.5rem] p-6 ${dark ? 'bg-slate-900/60 border-slate-800/50' : 'bg-white/70 border-slate-200/50'} backdrop-blur-xl border shadow-2xl transition-all duration-500`}
 >
 ...
 deviceType === 'PC' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 
 ...
 <div>
 <div className={`text-[10px] font-bold ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
 ...
 <div className={`text-[10px] font-bold ${dark ? 'text-slate-500' : 'text-slate-400'}`}>{t.nosVersion}</div>
 ...
 <div className="flex flex-col">
 <div className={`text-[10px] font-bold ${dark ? 'text-slate-500' : 'text-slate-400'}`}>{t.model}</div>
 <div className={`text-sm font-bold ${dark ? 'text-slate-200' : 'text-slate-700'}`}>{state.version.modelName}</div>
 </div>
 
 <div className="flex flex-col">
 <div className={`text-[10px] font-bold ${dark ? 'text-slate-500' : 'text-slate-400'}`}>{t.activePorts}</div>
