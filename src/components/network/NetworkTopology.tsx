...
export function NetworkTopology({
 ...
 const canvasDims = getCanvasDimensions();
 const clampedX = Math.max(50, Math.min(snappedX, canvasDims.width - 120));
 const clampedY = Math.max(50, Math.min(snappedY, canvasDims.height - 150));
 ...
 <div className={`text-xs font-bold ${isDark ? 'text-slate-500' : 'text-slate-400'} whitespace-nowrap`}>
 {t.devices}
 </div>
 ...
 <p className="text-xs font-bold text-slate-500 ml-1">{t.devices}</p>
 ...
 {/* Cables Section */}
 <div className="space-y-4">
 <p className="text-xs font-bold text-slate-500 ml-1">{t.cableTypes}</p>
 <div className="grid grid-cols-3 gap-3">
 {(['straight', 'crossover', 'console'] as CableType[]).map((type) => (
 <button
 key={type}
 ...
 className={`flex items-center justify-between px-2 text-xs font-semibold cursor-move select-none ${isDark ? 'bg-black/10' : 'bg-black/5'
 }`}
 ...
 <div className={`text-[10px] font-bold opacity-60 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
 ...
 <span className={`text-[10px] font-black ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
 ...
 <label className={`text-[10px] font-black ml-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
 ...
 <div className={`text-xs font-black mb-2 opacity-70 ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>
 {t.deviceInfo}
 </div>
 ...
 <div className={`text-xs font-black ${isMobile ? 'mb-3' : 'mb-4'} opacity-70 ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>
 {language === 'tr' ? 'IP Yapılandırması' : 'IP Configuration'}
 </div>

 <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-3`}>
 <div className="space-y-1">
 <label className={`text-xs font-bold ml-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
 {t.ipAddress}
 </label>
 <input
 type="text"
 ...
 className={`flex-1 py-3.5 rounded-2xl text-xs font-black transition-all duration-300 border ${isDark
 ...
 className="flex-1 py-3.5 rounded-2xl text-xs font-black bg-cyan-500 text-white hover:bg-cyan-400 shadow-xl shadow-cyan-500/20 active:scale-95 transition-all duration-300"
 >
 ...
 <span className={`px-1.5 py-0.5 rounded text-[9px] font-black ${port.status === 'connected' ? 'bg-emerald-500/20 text-emerald-500' :
