...
export function NetworkTopologyPortSelectorModal({
 ...
 className={`px-4 py-2 rounded-lg text-[10px] font-black transition-all duration-300 ${cableType === type
 ...
 <div className={`text-xs font-bold ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>
 {portSelectorStep === 'source' ? t.step1 : t.step2}
 </div>
 <button
 onClick={onClose}
 className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-xs font-black transition-all ${isDark ? 'bg-slate-800 text-slate-400 hover:text-slate-200' : 'bg-slate-100 text-slate-500 hover:text-slate-700'}`}
 >
 {t.cancel}
 </button>
 </div>
