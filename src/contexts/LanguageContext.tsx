...
export function LanguageProvider({ children }: { children: ReactNode }) {
 ...
 // İlk mount'ta localStorage'dan veya sistem dilinden dil yükle
 useEffect(() => {
 ...
 // Sistem dilini kontrol et, tr değilse en (English) olarak ayarla
 const browserLang = navigator.language.split('-')[0];
 setLanguage(browserLang === 'tr' ? 'tr' : 'en');
 }
 } catch {
 // localStorage erişim hatası - varsayılan en kullan
 setLanguage('en');
 }
 setInitialized(true);
 }, [initialized]);
