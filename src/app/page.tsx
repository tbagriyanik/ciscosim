...
export default function Home() {
 ...
 const handleUpdateDeviceName = useCallback((id: string, name: string) => {
 setTopologyDevices(prev => prev.map(d => d.id === id ? { ...d, name } : d));
 // Also update deviceStates if exists
 setDeviceStates(prev => {
 const state = prev.get(id);
 if (state) {
 return new Map(prev).set(id, { ...state, hostname: name });
 }
 return prev;
 });
 }, [setTopologyDevices, setDeviceStates]);

 const handleCommand = useCallback(async (deviceId: string, command: string) => {
 ...
 if (command.toLowerCase().startsWith('hostname ')) {
 const name = command.substring(9).trim();
 handleUpdateDeviceName(deviceId, name);
 }
 }, [handleCommandForDevice, topologyDevices, topologyConnections, setActiveDeviceId, setActiveDeviceType, handleUpdateDeviceName]);
 ...
 <span className={`text-[10px] font-black ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
 {t.labProgress}
 </span>
 <motion.span
 key={totalScore}
 ...
 <p className="text-xs font-bold text-slate-500 mb-2 px-1">
 ...
 <p className="text-xs font-bold text-slate-500 px-2 mb-1">{t.navigation}</p>
 ...
 <span className="text-xs font-bold text-slate-500">{t.labProgress}</span>
 ...
 <DropdownMenuLabel className="text-[11px] font-bold text-slate-500 py-2">
 ...
 <span className={`text-[10px] ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>{example.tag}</span>
 </div>
 ...
 onExecuteCommand={(cmd) => handleExecuteCommand(activeDeviceId, cmd)}
 ...
 <ConfigPanel
 state={state}
 onExecuteCommand={handleCommand.bind(null, activeDeviceId)}
 onUpdateDeviceName={handleUpdateDeviceName}
 isDevicePoweredOff={topologyDevices.some(d => d.id === activeDeviceId && d.status === 'offline')}
 t={t}
 theme={theme}
